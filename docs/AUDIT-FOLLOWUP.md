...
- **E2E Playwright — ✅ fait (local).** `playwright.config.ts` + `e2e/` : `auth.setup.ts` (login →
  ...
  **✅ Câblé en CI** : job `e2e` (`.github/workflows/ci.yml`) sur les **PR (même dépôt)
  et au push sur `main`** ; `TEST_EMAIL`/`TEST_PASSWORD` en secrets de dépôt, `VITE_*`
  publiques dans le workflow, PR de fork exclues (voir `e2e/README.md`).
