/**
 * Référentiel Thermomix TM7 — la « base de référence » machine de l'app.
 *
 * SOURCE DE VÉRITÉ (edge/Deno). Un miroir strictement identique existe côté
 * front : `src/lib/thermomix/reference.ts`. Deno ne pouvant pas importer depuis
 * `src/`, les deux fichiers sont maintenus à la main — un test de garde
 * (`src/lib/thermomix/reference.sync.test.ts`) échoue si les données divergent.
 *
 ...
 * fonctions pures de normalisation/validation. Réutilisé par :
 *  - le prompt IA (`home-assistant`) pour générer des étapes au format TM7,
 *  - la validation du payload avant export,
 *  - le mapper Cookidoo (construction des annotations « guided cooking »).
 *
 * Aucune dépendance (TS pur) → importable en Deno, Node (CLI) et Vite (front).
 *
