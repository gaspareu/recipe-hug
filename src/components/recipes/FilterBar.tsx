    ...
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        <Select value={statusFilter} onValueChange={(value) => onStatusFilterChange(value as RecipeStatus | 'all')}>
          <SelectTrigger id="recipe-status-filter"><SelectValue /></SelectTrigger>
          <SelectContent>
      ...
      <div className="space-y-2">
        ...
        <Select value={sortBy} onValueChange={(value) => onSortByChange(value as RecipeBookSort)}>
          <SelectTrigger id="recipe-sort"><SelectValue /></SelectTrigger>
          <SelectContent>
  ...
  return (
    <div className="space-y-3" aria-label="Filtres du livre">
      <div className="relative">
        ...
        <Input placeholder="Rechercher une recette..." value={search} onChange={(event) => onSearchChange(event.target.value)} className="border-[hsl(var(--book-ink)/0.25)] bg-transparent pl-9" />
      </div>

      <div className="flex flex-wrap items-center gap-2">
        <Button type="button" variant={hasActiveFacets ? 'outline' : 'secondary'} size="sm" onClick={onShowAll} aria-pressed={!hasActiveFacets}>Toutes</Button>
        <Select value={seasonFilter} onValueChange={onSeasonFilterChange}>
          <SelectTrigger className="h-9 w-auto min-w-32" aria-label="Saison"><Leaf className="mr-1.5 h-4 w-4" aria-hidden="true" /><SelectValue placeholder="Saison" /></SelectTrigger>
          <SelectContent>
        ...
        <div className="sm:hidden">
          <Sheet>
            <SheetTrigger asChild><Button type="button" variant="outline" size="sm" aria-label="Filtres et tri"><SlidersHorizontal className="mr-1.5 h-4 w-4" aria-hidden="true" />Filtres et tri{activeControlCount > 0 ? ` (${activeControlCount})` : ''}</Button></SheetTrigger>
            <SheetContent side="bottom"><SheetHeader><SheetTitle>Filtres et tri</SheetTitle></SheetHeader><div className="mt-6">{secondary}</div></SheetContent>
          </Sheet>
        </div>
        <div className="hidden sm:block">
          <Popover>
            <PopoverTrigger asChild><Button type="button" variant="outline" size="sm" aria-label="Filtres et tri"><Filter className="mr-1.5 h-4 w-4" aria-hidden="true" />Filtres et tri{activeControlCount > 0 ? ` (${activeControlCount})` : ''}</Button></PopoverTrigger>
            <PopoverContent align="end">{secondary}</PopoverContent>
          </Popover>
        </div>

        {hasActiveFilters && <Button type="button" variant="ghost" size="sm" onClick={onClearAll} aria-label="Réinitialiser les filtres">✕ <span className="sr-only">Réinitialiser les filtres</span></Button>}
      </div>
    </div>
  );
}
