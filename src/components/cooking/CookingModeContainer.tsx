...
export function CookingModeContainer({ recipeId, onClose, initialServings }: CookingModeContainerProps) {
  ...
  const handleRecipeUpdate = useCallback(
    async (data: { title: string; servings: number; ingredients: Ingredient[]; steps: Step[] }) => {
      await updateRecipe.mutateAsync({ id: recipeId, title: data.title, servings: data.servings, ingredients: data.ingredients, steps: data.steps });
      await refetch();
    },
  ...
  const handleRecipeCreate = useCallback(
    async (data: { title: string; servings: number; ingredients: Ingredient[]; steps: Step[]; relationToOriginal?: string }) => {
