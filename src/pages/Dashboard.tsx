...
import {
  ...
  recipeBookSearch,
  serializeRecipeBookFilters,
  type RecipeBookFilters,
} from '@/lib/recipe-book-filters';
...
export default function Dashboard() {
  ...
  const updateFilters = (next: Partial<RecipeBookFilters>) => {
    setSearchParams(serializeRecipeBookFilters({ ...filters, ...next }), { replace: true });
  };
    ...
    <MainLayout>
      <BookPageFrame className="space-y-5">
        <header className="flex flex-wrap items-start justify-between gap-3 border-b border-[hsl(var(--book-ink)/0.18)] pb-4">
          <div>
            <h1 className="font-solitreo text-3xl">Mes recettes</h1>
            <p className="mt-1 text-sm text-[hsl(var(--book-ink)/0.72)]">
              {filteredRecipes.length} sur {recipes.length} recette{recipes.length !== 1 ? 's' : ''}
            </p>
          </div>
          <Button asChild className="bg-[hsl(var(--book-bookmark))] text-[hsl(var(--book-bookmark-foreground))] hover:bg-[hsl(var(--book-bookmark)/0.85)]">
            <Link to={`/recipes/new${query}`}>
        ...
        <FilterBar
          ...
          onFavoritesOnlyChange={(favorites) => updateFilters({ favorites })}
          seasonFilter={filters.season}
          onSeasonFilterChange={(season) => updateFilters({ season: season as RecipeBookFilters['season'] })}
          sortBy={filters.sort}
          onSortByChange={(sort) => updateFilters({ sort })}
          onShowAll={showAll}
          onClearAll={resetAll}
        />
        {Boolean(recipeBookSearch(filters)) && (
          <Button type="button" variant="link" className="h-auto px-0 text-[hsl(var(--book-ink))]" onClick={resetAll}>
        ...
        <RecipeIndex
          ...
          onToggleFavorite={(id, is_favorite) => toggleFavorite.mutate({ id, is_favorite })}
